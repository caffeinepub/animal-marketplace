...
export default function Layout({ children }: LayoutProps) {
  ...
  useInternetIdentity(); // keep hook for session initialization side-effects
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <header className="bg-primary text-white sticky top-0 z-40 shadow-md" style={{ pointerEvents: 'auto' }}>
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full font-medium">v57</span>
            <button
              type="button"
              onClick={() => { window.location.href = '/admin'; }}
              className="flex items-center gap-1 text-xs bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg transition-colors border border-white/20 font-semibold"
      ...
      <footer className="bg-white border-t border-gray-200 py-4 text-center text-xs text-gray-500 pb-24">
        <p>© {new Date().getFullYear()} Animal Pashu Bazar — Version 57</p>
        <p className="mt-1">
