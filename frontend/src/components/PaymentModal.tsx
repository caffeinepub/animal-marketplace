...
import { Copy, Check, QrCode, IndianRupee, Loader2, AlertCircle, Tag, CheckCircle2, Crown } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const UPI_ID = '8431207976king@ybl';
...
export default function PaymentModal({
  ...
  // Promo codes only apply to non-VIP ads
  const effectivePrice = (!isVip && promoApplied) ? DISCOUNTED_PRICE : basePrice;
          ...
          <div className="space-y-5">
            ...
            <div className={`rounded-xl px-5 py-4 flex items-center justify-between ${
              ...
              <div className="flex items-center gap-2">
                {!isVip && promoApplied && (
                  <span className="text-muted-foreground line-through text-base font-medium">₹{REGULAR_PRICE}</span>
                )}
                <span className={`font-bold text-2xl font-display ${isVip ? 'text-amber-600' : 'text-primary'}`}>
                  ₹{effectivePrice}
                </span>
              </div>
            </div>

            {/* Promo Code Section — only for non-VIP ads */}
            {!isVip ? (
              ...
              <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5 text-sm text-amber-700">
                ...
                <span>Promo codes are not applicable for VIP Ads.</span>
              </div>
            )}
